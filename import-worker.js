...
const doStoreLocal = function(dcmFile) {
	return new Promise(async function(resolve, reject){
		let storeCommand = 'curl -X POST --user demo:demo http://localhost:8042/instances --data-binary @' + dcmFile;
...
const doStorePACS = function(instanceId){
	return new Promise(async function(resolve, reject){
		let storeCommand = 'curl -X POST --user demo:demo http://localhost:8042/modalities/pacs/store -d ' + instanceId;
